...
//			this._matrix.d /= 2;
		...
		private function createFont(format : TextFormat, filters : Array) : void {
			...
			this._bmp = new BitmapData(512, 512, true, 0);
			var tmpMatrix : Matrix = new Matrix(1, 0, 0, 1, 0, 0);
			// 初始化96个默认字符
			var i : int = 32;
			while (i < 128) {
				textField.text = String.fromCharCode(i);

				if ((tmpMatrix.tx + textField.width) >= this._bmp.width)
				{ // 换行
					tmpMatrix.tx = 0;
			...
			var tMatrix:Matrix = new Matrix(1, 0, 0, 1, -2, 0);
			_tmpBmd.draw(this._bmp, tMatrix);
			this._bmp.dispose();
			this._bmp = _tmpBmd;
