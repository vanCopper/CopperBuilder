	...
	public class TabControl extends Control {
		...
		private function onClickedMenu(event:Event) : void {
			if (this._currentPanel) {
				this._menuBtn.view.contextMenu.display(App.stage, App.stage.mouseX, App.stage.mouseY);
			}
		}
...
class Tab extends Sprite {
	...
	private var fontText:TextField = new TextField();
	...
	public function Tab(tabcontrol : TabControl, panel : Panel) {
		...
		fontText.text 			= "|";
		fontText.autoSize 		= "left";
		fontText.antiAliasType = AntiAliasType.ADVANCED;
		fontText.defaultTextFormat = new TextFormat("Arial", 10, 0xB0B0B0, true);
		fontText.mouseEnabled = false;
		this._background = new Sprite();
		...
		this._foreground.addChild(fontText);
		this.draw();
	}
	...
	public function draw() : void {
		...
		fontText.text = this.panel.text;
		fontText.x = (_foreground.width - fontText.width)/2;
		fontText.y = (_foreground.height - fontText.height)/2;
